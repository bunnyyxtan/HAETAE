...
// Subject: src/examples/SandboxVerifier.sol + src/examples/TestUSDC.sol — the two
...
contract SandboxStackTest is Test {
    SandboxVerifier ver;
    TestUSDC usdc;
    ...
    function setUp() public {
        ver = new SandboxVerifier();
        usdc = new TestUSDC();
    }

    // --- SandboxVerifier: the whole point is that it never says no ---------------

    function test_Verifier_AlwaysTrue(address subject) public view {
        assertTrue(ver.isVerified(subject));
    }

    // --- TestUSDC: USDC-shaped, worthless by construction ----------------------
